...
public class ChutesDelete extends AsyncTask<Void, Void, Boolean> {
	...
	if (ownerStatus == ChuteOwnerStatus.OWNER) {
	    client = new ChuteRestClient(String.format(ChuteConstants.URL_CHUTES_DELETE, id));
	} else {
	    client = new ChuteRestClient(String.format(ChuteConstants.URL_CHUTES_LEAVE, id));
	}
	try {
